"""Shared constants, paths, word lists, and HTTP/NLP helpers for the RQ3 pipeline."""
...
# Display labels used on charts. Keys are the canonical Moltbook usernames
# (also used as filenames on disk); values are the model-variant friendly
# names the figures and CSVs should render. The control agent stays
# "control" so the rq3 visuals match the convention used in rq2 and rq4.
DISPLAY_LABELS: dict[str, str] = {
    "baselineagent": "M-gemini 2.5 flash (Control)",
    "opus_claw": "M-Opus",
    "sonnetclaw": "M-Sonnet",
    "gpt4oclaw": "M-gpt5.4",
    "qwen_claw": "M-Qwen",
...
def display_label(agent: str) -> str:
    """Return the persona-friendly label for an agent username, fallback to title-cased name."""
    if agent in DISPLAY_LABELS:
        return DISPLAY_LABELS[agent]
...
class MoltbookClient:
    ...
    def _headers(self, force_auth: bool) -> dict[str, str]:
        headers = {"Accept": "application/json", "User-Agent": "openclaw-rq3/1.0"}
