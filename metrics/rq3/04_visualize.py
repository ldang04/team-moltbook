...
def chart_word_count(rows: list[dict[str, Any]]) -> Path:
    ...
    fig.tight_layout()
    out = FIGURES / "rq3_word_count.pdf"
    fig.savefig(out)
    plt.close(fig)
    return out
...
def chart_linguistic_markers(rows: list[dict[str, Any]]) -> Path:
    ...
    fig.tight_layout()
    out = FIGURES / "rq3_linguistic_markers.pdf"
    fig.savefig(out)
    plt.close(fig)
    return out
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Render RQ3 figures.")
    ap.parse_args()
